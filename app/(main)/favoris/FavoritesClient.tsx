...
export default function FavoritesClient({ events }: { events: FestivalEvent[] }) {
  ...
  return (
    <>
      <section className="border-b border-[#252525] px-8 py-20 text-center md:px-12">
        <span className="font-condensed text-xs uppercase tracking-[0.35em] text-pink">
          Votre selection
        </span>
        <h1 className="mt-3 font-display text-[clamp(58px,14vw,110px)] uppercase leading-none text-white">
          Mes favoris
        </h1>
      </section>

      <section className="border-b border-[#252525] px-6 py-5 md:px-10">
      ...
      {favorites.length > 0 ? (
        <section className="grid grid-cols-1 divide-y divide-[#252525] md:grid-cols-2 md:divide-x md:divide-y-0 xl:grid-cols-3">
          {favorites.map((event) => (
              ...
              <Link href={`/programme/${event.id}`} className="block">
                <div className="relative h-[380px]">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <h2 className="mt-2 font-display text-5xl uppercase text-white">{event.artist}</h2>
        ...
        <section className="px-8 py-24 text-center">
          <h2 className="font-display text-5xl uppercase text-white">Aucun favori</h2>
          <p className="mx-auto mt-4 max-w-md text-sm leading-6 text-white/50">
          ...
          <Link
            href="/programme"
            className="mt-8 inline-flex border-2 border-lime bg-lime px-8 py-4 font-condensed text-xs font-bold uppercase tracking-[0.2em] text-dark transition-all hover:bg-[#111113] hover:[color:var(--neon-lime)]"
          >
            Voir le programme
          </Link>
        </section>
